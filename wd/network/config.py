...
def torch_version_float():
    ...
    if version_re:
        ...
        logger.info(f'Torch version: {version}, {version_str}')
    else:
        version = 1.0
        logger.error(f'Can\'t parse torch version ({version}), assuming {version}')
    return version
...
def assert_and_infer_cfg(args, make_immutable=True, train_mode=True):
    ...
    if args.dump_assets and args.dataset == 'cityscapes':
        ...
        logger.warning('*' * 70)
        logger.warning(f'ALERT: forcing cv=3 to allow all images to be evaluated')
        logger.warning('*' * 70)
        cfg.DATASET.CV = 3
    else:
    ...
    if args.n_scales:
        ...
        logger.info('n scales {}'.format(cfg.MODEL.N_SCALES))
